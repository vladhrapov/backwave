  ...
  completeRoutes =[]; //найденные полные пути
      ...
      matrix[this.startVertex].forEach((item, index) => {
        ...
        if (item && item.weight) {
            ...
            startingRoutes[i] = {"path":[],"routeWeight":0, "routeIndex": i};//при удаление из этого массива смещается индекс, поэтому запоминаем (понадобится)
            startingRoutes[i].path.push({"name": `A${this.startVertex + 1}`, "routeWeight":0, "index": this.startVertex, "pow": 0, "pathIndex": i}, item);
...
while(startingRoutes.length>0)
{
      ...
      startingRoutes.forEach((item, index) => {
        ...
        {
          let leastPowVertex = {"pow": Number.POSITIVE_INFINITY}; //Вершина с найменьшей степенью, которую будем рассматривать
          ...
          let leastWeightLink = {"weight": Number.POSITIVE_INFINITY}; //смежная вершина с найменьшей стоимостью
